...
mod tests {
    ...
    #[test]
    fn parse_cli_args_from_parses_local_vm_execution_mode() {
        let action = parse_cli_args_from(["--vm-execution-mode".to_string(), "local".to_string()])
            .expect("parse should succeed");

        let CliAction::Run(cli) = action else {
            panic!("expected run action");
        };
        assert_eq!(cli.vm_execution_mode, Some(VmExecutionMode::Local));
    }
