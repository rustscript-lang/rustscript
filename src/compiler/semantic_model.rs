...
pub struct SemanticModel {
    ...
    /// The resolved module graph used to build this model, when compilation
    /// went through the module loader.
...
impl SemanticModel {
    ...
    /// Build a semantic model and retain the resolved module graph that
    /// produced it. Language-service clients can use this graph to invalidate
    /// dependent documents without reconstructing import syntax.
    pub fn new_with_module_graph(
        ...
        let mut model = Self::new(ir, sources, catalog, errors);
        model.module_graph = Some(module_graph);
        model
    }
    ...
    /// The resolved module graph used during module-aware compilation.
    ...
    /// signature-help display. The documentation comes from the catalog entry
    /// with the complete import identity, never from a name-only lookup.
    fn resolved_call_to_host_schema(&self, resolved: &ResolvedHostCall) -> HostFunctionSchema {
        let import = self.resolved_call_to_host_import(resolved);
        let description = self
            .catalog
            .function_for_import(&import)
            .map(|function| function.description.clone())
            .unwrap_or_default();
        let HostImportSchema {
            name,
            params,
            return_type,
            ..
        ...
        HostFunctionSchema {
            ...
            params: params
                .into_iter()
                .map(|param| crate::host_api::HostParamSchema {
                    name: param.name,
                    ty: param.schema,
    ...
    /// Convert a resolved host call to its complete catalog-import identity.
    /// Keeping this as a first-class value makes resource keys, passing modes,
    /// return schemas and catalog provenance impossible to omit from metadata
    /// lookups.
    fn resolved_call_to_host_import(&self, resolved: &ResolvedHostCall) -> HostImportSchema {
        HostImportSchema {
            ...
            params: resolved
                ...
                .map(|(param, passing)| HostImportParam {
                    ...
                    schema: self.compiler_schema_to_host_schema(&param.schema),
                    passing: *passing,
                })
                .collect(),
    ...
    /// schema identity carried on the call (canonical name plus the complete
    /// parameter/return schema), not a name-only catalog scan.
    fn host_definition_for_call(
        &self,
        info: &crate::compiler::ir::ResolvedCallInfo,
    ) -> Option<Definition> {
        let resolved = info.host.as_ref()?;
        let schema = self.resolved_call_to_host_schema(resolved);
        let discriminator = schema.identity_discriminator();
        let key = format!(
            "host://{}/{}/{}",
            schema.name,
            schema.params.len(),
