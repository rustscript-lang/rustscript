...
pub(super) struct ParsedUnit {
    ...
    /// Whether this unit was parsed without an explicit host catalog. This
    /// distinguishes legacy builtin metadata from catalog-backed host imports.
    pub(super) host_catalog_supplied: bool,
}
...
fn register_unit_functions(
    ...
    for func in &unit.parsed.functions {
        ...
        let flat = if !has_impl {
            // Legacy stdlib modules expose builtin names through synthetic
            // declarations. Without an explicit host catalog, preserve the
            // builtin call index instead of materializing a host import.
            if !unit.host_catalog_supplied
                && let Some(builtin) = BuiltinFunction::from_namespaced_name(&func.name)
            {
                let builtin_index = builtin.call_index();
                flat_index_by_symbol.insert(symbol, builtin_index);
                map.insert(func.index, builtin_index);
                continue;
            }
