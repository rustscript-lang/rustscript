...
impl TypeSchema {
    ...
    pub(crate) fn contains_resource_with_named_types(
        &self,
        ...
        fn substitute(schema: &TypeSchema, bindings: &HashMap<String, TypeSchema>) -> TypeSchema {
            match schema {
                ...
                TypeSchema::Array(inner) => {
                    TypeSchema::Array(Box::new(substitute(inner, bindings)))
                }
                ...
                TypeSchema::Map(value) => TypeSchema::Map(Box::new(substitute(value, bindings))),
        ...
        // Recursive generic arguments can grow on every re-entry (for example
        // `Node<T> { child: Node<[T]> }`), so an active-instantiation set alone
        // does not bound the walk. Count expansions per declaration identity;
        // distinct named declarations still get their own independent budget.
        const MAX_NAMED_TYPE_REENTRIES: usize = 64;

        fn visit(
            schema: &TypeSchema,
            structs: &HashMap<String, StructDecl>,
            active: &mut HashSet<(String, Vec<TypeSchema>)>,
            reentries: &mut HashMap<String, usize>,
        ) -> bool {
            match schema {
                TypeSchema::Resource(_) => true,
                TypeSchema::Optional(inner) | TypeSchema::Array(inner) | TypeSchema::Map(inner) => {
                    visit(inner, structs, active, reentries)
                }
                TypeSchema::ArrayTuple(items) => items
                    .iter()
                    .any(|item| visit(item, structs, active, reentries)),
                TypeSchema::ArrayTupleRest { prefix, rest } => {
                    prefix
                        .iter()
                        .any(|item| visit(item, structs, active, reentries))
                        || visit(rest, structs, active, reentries)
                }
                TypeSchema::Object(fields) => fields
                    .values()
                    .any(|value| visit(value, structs, active, reentries)),
                TypeSchema::Callable { params, result } => {
                    params
                        .iter()
                        .any(|param| visit(param, structs, active, reentries))
                        || visit(result, structs, active, reentries)
                }
                TypeSchema::Named(name, args) => {
                    if args
                        .iter()
                        .any(|arg| visit(arg, structs, active, reentries))
                    {
                        return true;
                    }
                    let Some(decl) = structs.get(name) else {
                        return false;
                    };
                    ...
                    let count = reentries.get(name).copied().unwrap_or(0);
                    if count >= MAX_NAMED_TYPE_REENTRIES {
                        // The remaining shape is unknown, so conservatively
                        // classify it as resource-bearing rather than risk a
                        // false negative in ownership analysis.
                        active.remove(&key);
                        return true;
                    }
                    reentries.insert(name.clone(), count + 1);
                    let bindings = decl
                        .type_params
                        .iter()
                        .cloned()
                        ...
                        .collect::<HashMap<_, _>>();
                    let body = substitute(&decl.body_schema, &bindings);
                    let contains = visit(&body, structs, active, reentries);
                    if count == 0 {
                        reentries.remove(name);
                    } else {
                        reentries.insert(name.clone(), count);
                    }
                    active.remove(&key);
                    contains
                }
        ...
        visit(self, structs, &mut HashSet::new(), &mut HashMap::new())
    }
...
mod type_schema_contains_resource_tests {
    ...
    #[test]
    fn named_struct_body_recursed_for_resource_ownership() {
        ...
        assert!(wrapper.contains_resource_with_named_types(&structs));
    }

    #[test]
    fn named_generic_struct_substitutes_resource_argument() {
        let structs = HashMap::from([(
            "wrapper".to_string(),
            StructDecl {
                name: "wrapper".to_string(),
        ...
        let scalar_wrapper = TypeSchema::Named("wrapper".to_string(), vec![TypeSchema::Int]);
        assert!(resource_wrapper.contains_resource_with_named_types(&structs));
        assert!(!scalar_wrapper.contains_resource_with_named_types(&structs));
    }

    #[test]
    fn growing_generic_recursion_is_bounded_conservatively() {
        let structs = HashMap::from([(
            "node".to_string(),
            StructDecl {
                name: "node".to_string(),
        ...
        assert!(node.contains_resource_with_named_types(&structs));
    }
