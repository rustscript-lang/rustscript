...
/// Converts a compiler schema back to the host-facing schema retained in the
/// VMBC sidecar. Catalog-resolved host signatures only contain the representable
/// host subset; compiler-only nominal schemas conservatively become `Unknown`.
pub(crate) fn to_host_schema(schema: &TypeSchema) -> HostTypeSchema {
    match schema {
        TypeSchema::Unknown | TypeSchema::GenericParam(_) => HostTypeSchema::Unknown,
        ...
        TypeSchema::Optional(inner) => HostTypeSchema::Optional(Box::new(to_host_schema(inner))),
        TypeSchema::Array(inner) => HostTypeSchema::Array(Box::new(to_host_schema(inner))),
        TypeSchema::ArrayTuple(items) => HostTypeSchema::Array(Box::new(
            items
                .first()
                .map(to_host_schema)
                .unwrap_or(HostTypeSchema::Unknown),
        )),
        TypeSchema::ArrayTupleRest { rest, .. } => {
