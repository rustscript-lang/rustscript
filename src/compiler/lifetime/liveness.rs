...
mod tests {
    use super::*;
    use crate::compiler::ir::MatchPattern;
    use crate::compiler::modules::{ModuleId, SymbolId};

    fn boxed(slot: LocalSlot) -> Box<Expr> {
        Box::new(Expr::Var(slot))
    }

    fn closure() -> ClosureExpr {
        ClosureExpr {
            param_slots: vec![5],
            capture_copies: vec![(0, 4)],
            body: Box::new(Expr::LocalCall(5, Vec::new(), vec![Expr::Var(1)])),
        }
    }

    fn representative_exprs() -> Vec<(&'static str, Expr)> {
        vec![
            ("Null", Expr::Null),
            ("Int", Expr::Int(1)),
            ("Float", Expr::Float(1.0)),
            ("Bool", Expr::Bool(true)),
            ("String", Expr::String("text".to_string())),
            ("Bytes", Expr::Bytes(vec![1, 2, 3])),
            ("FunctionRef", Expr::FunctionRef(0, Vec::new())),
            (
                "ModuleFunctionRef",
                Expr::ModuleFunctionRef(
                    SymbolId {
            ...
            ("Call", Expr::Call(0, Vec::new(), vec![Expr::Var(1)])),
            (
                "ModuleCall",
                Expr::ModuleCall(
                    SymbolId {
            ...
            (
                "LocalCall",
                Expr::LocalCall(3, Vec::new(), vec![Expr::Var(4)]),
            ),
            ("Closure", Expr::Closure(closure())),
            (
                "ClosureCall",
                Expr::ClosureCall(closure(), vec![Expr::Var(6)]),
            ),
            ("Add", Expr::Add(boxed(0), boxed(1))),
            ("Sub", Expr::Sub(boxed(0), boxed(1))),
            ("Mul", Expr::Mul(boxed(0), boxed(1))),
            ("Div", Expr::Div(boxed(0), boxed(1))),
            ("Mod", Expr::Mod(boxed(0), boxed(1))),
            ("Neg", Expr::Neg(boxed(0))),
            ("Not", Expr::Not(boxed(0))),
            ("And", Expr::And(boxed(0), boxed(1))),
            ("Or", Expr::Or(boxed(0), boxed(1))),
            ("Eq", Expr::Eq(boxed(0), boxed(1))),
            ("Lt", Expr::Lt(boxed(0), boxed(1))),
            ("Gt", Expr::Gt(boxed(0), boxed(1))),
            ("Var", Expr::Var(0)),
            ("MoveVar", Expr::MoveVar(1)),
            (
                "MoveField",
                Expr::MoveField {
                    root: 2,
            ...
            ("MoveIndex", Expr::MoveIndex { root: 3, index: 1 }),
            (
                "IfElse",
                Expr::IfElse {
                    condition: boxed(0),
            ...
            (
                ...
                Expr::Match {
                    ...
                    value: boxed(0),
                    arms: vec![
                        (MatchPattern::Int(1), Expr::Var(1)),
                        (MatchPattern::SomeBinding(5), Expr::MoveVar(6)),
                    ],
                    default: boxed(2),
                },
            ),
            ("ToOwned", Expr::ToOwned(boxed(0))),
            ("Borrow", Expr::Borrow(boxed(0))),
            ("BorrowMut", Expr::BorrowMut(boxed(0))),
            (
                "Block",
                Expr::Block {
                    stmts: vec![Stmt::Let {
    ...
    #[test]
    fn precise_liveness_is_subset_for_every_expr_variant_and_drops_stay_conservative() {
        let function_impls = HashMap::new();
        let rewriter = LivenessRewriter::new(8, &[], &function_impls);
        let expressions = representative_exprs();
        assert_eq!(
            expressions.len(),
            38,
            "update this exhaustive list when Expr gains a variant"
        );
        for (variant, expr) in expressions {
            let precise = rewriter.uses_expr_precise(&expr);
            let conservative = rewriter.uses_expr(&expr);
            assert_eq!(precise.len(), conservative.len(), "variant {variant}");
            assert!(
                precise
                    .iter()
                    .zip(&conservative)
                    .all(|(precise, conservative)| !precise || *conservative),
                "precise uses must be a subset of conservative uses for {variant}: precise={precise:?}, conservative={conservative:?}"
            );
        }

        let rewritten = rewriter.rewrite_program_block(&[
            Stmt::Let {
                index: 0,
                declared_schema: None,
                expr: Expr::String("captured".to_string()),
                line: 1,
            },
            Stmt::ClosureLet {
                line: 2,
                closure: ClosureExpr {
                    param_slots: vec![2],
                    capture_copies: vec![(0, 3)],
                    body: Box::new(Expr::Var(2)),
                },
            },
            Stmt::Expr {
                expr: Expr::LocalCall(3, Vec::new(), Vec::new()),
                line: 3,
            },
        ]);
        let call_position = rewritten
            .iter()
            .position(|stmt| {
                matches!(
                    stmt,
                    Stmt::Expr {
                        expr: Expr::LocalCall(3, _, _),
                        ..
                    }
                )
            })
            .expect("dynamic local call should remain in rewritten block");
        assert!(
            !rewritten[..call_position]
                .iter()
                .any(|stmt| matches!(stmt, Stmt::Drop { index: 0, .. })),
            "conservative drop insertion must keep a captured source live until a dynamic call"
        );
    }
}
