...
fn explicit_host_api_catalog(options: &CompileSourceFileOptions) -> Option<Arc<HostApiCatalog>> {
    options.host_api_catalog().map(Arc::clone)
}
...
pub(super) fn parse_source_for_compile(
    ...
    parse_source_with_source_id_and_externs_with_catalog(
        source,
        flavor,
        options,
        0,
        false,
        explicit_host_api_catalog(options),
    )
}
...
fn parse_source_with_source_id_and_externs_with_catalog(
    source: &str,
    flavor: SourceFlavor,
    options: &CompileSourceFileOptions,
    original_source_id: u32,
