...
/// The typed per-VM module-state store owned by a host runtime.
///
/// This is the generic host-owned policy/configuration storage the SDK's
/// [`HostContext::set_module_state`] family drives. State is opaque to the VM,
/// keyed by [`TypeId`], and deliberately survives scope reset (the HTTP host
/// configuration is the same-crate precedent).
pub(crate) struct ModuleStateStore {
    entries: HashMap<TypeId, Box<dyn Any + Send>>,
}

impl Default for ModuleStateStore {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ModuleStateStore {
    ...
    pub(crate) fn get<M: HostModule>(&self) -> Option<&M> {
        self.entries
            .get(&TypeId::of::<M>())
            .and_then(|state| state.downcast_ref::<M>())
    }

    pub(crate) fn get_mut<M: HostModule>(&mut self) -> Option<&mut M> {
        self.entries
            .get_mut(&TypeId::of::<M>())
            .and_then(|state| state.downcast_mut::<M>())
    }

    pub(crate) fn remove<M: HostModule>(&mut self) -> Option<M> {
        self.entries
            .remove(&TypeId::of::<M>())
            .and_then(|state| state.downcast::<M>().ok())
            .map(|state| *state)
    }
