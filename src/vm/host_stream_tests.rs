...
use super::{HostStreamAction, HostStreamDriver, HostStreamPoll};
use crate::vm::operation::reason::OperationCancelReason;
use crate::{
    CallOutcome, CallReturn, HostAsyncBridge, HostFunction, HostFuture, HostFutureOutput, HostOpId,
    InvocationError, InvocationItem, InvocationPoll, JitConfig, Value, Vm, VmError, VmMap,
...
struct CancellationLog {
    ...
    cancellations: Vec<(HostOpId, OperationCancelReason)>,
}
...
fn invocation_cancellation_during_callback_wait_releases_the_stream_driver() {
    ...
    let callable = vm.resolve_exported_callable("run").unwrap();
    {
        ...
        assert_eq!(polls.load(Ordering::SeqCst), 1);
        invocation.cancel(OperationCancelReason::Requested).unwrap();
        assert!(matches!(
            invocation.poll_next().unwrap(),
...
fn dropping_invocation_during_callback_wait_cancels_once_and_reuses_the_vm() {
    ...
    assert_eq!(cancelled[0].1, OperationCancelReason::Requested);
    let late = vm
...
fn explicit_cancel_then_drop_cancels_callback_wait_and_driver_once() {
...

    {
        ...
        invocation.cancel(OperationCancelReason::Deadline).unwrap();
    }
    ...
    let cancelled = &cancellations
        ...
        .cancellations;
    assert_eq!(cancelled.len(), 1);
    assert_eq!(cancelled[0].1, OperationCancelReason::Deadline);
}
