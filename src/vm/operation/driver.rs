...
//! scope that owns the operation). Drivers implement the concrete
//! [`HostOperation::cancel`] action; the registry records the first
...
pub trait HostOperation: Any + Send + 'static {
    ...
    /// while a detached worker still owns resources. Drivers without a
    /// background worker must explicitly return `true`; the fail-closed
    /// default prevents a worker-bearing driver from being released merely
    /// because it reached a terminal status.
...
impl OperationSpec {
    /// Builds a spec from a driver, leaving deadline/cleanup unset.
