...
mod tests {
    ...
    fn decode_rejects_invalid_encodings() {
        assert!(OperationId::from_raw(0).is_err());
        assert!(OperationId::from_raw(pack(1, 1, 1) | SIGN_MASK).is_err());
        assert!(OperationId::from_raw(pack(0, 1, 1)).is_err());
        assert!(OperationId::from_raw(pack(1, 0, 1)).is_err());
        assert!(OperationId::from_raw(pack(1, 1, 0)).is_err());
    }
