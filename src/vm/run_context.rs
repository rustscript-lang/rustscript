...
/// Run-scoped budgets, deadlines, and interruption state.
///
/// Thread safety: `RunContext` is `!Sync` (counters are mutable) and not
...
impl RunContext {
    /// Creates a fresh run context with no budgets (interrupts disabled).
    ...
    /// Closes run-scoped state for reuse: fuel/epoch budgets are dropped and
    /// rearm state is cleared, so metering is disabled on the next run.
