...
impl Vm {
    pub(super) fn jit_diagnostics_snapshot(&self) -> JitSnapshot {
        self.engine.jit.snapshot(self.jit_diagnostics_metrics())
    }
    ...
    pub(super) fn jit_diagnostics_dump(&self, include_machine_code: bool) -> String {
        ...
        out.push_str(&format!(
            ...
            self.engine.jit_native_compile_time_ns, self.engine.jit_native_region_compile_time_ns
        ));
        out.push_str(&format!(
        ...
        let native_trace_count = self.engine.native_traces.iter().flatten().count();
        ...
        for (id, native) in self.engine.native_traces.iter().enumerate() {
    ...
    fn jit_diagnostics_metrics(&self) -> JitMetrics {
        JitMetrics {
            ...
            trace_exit_count: self.engine.jit_trace_exit_count,
            native_loop_back_count: self.engine.jit_native_loop_back_count,
            helper_fallback_count: self.engine.jit_helper_fallback_count,
            native_trace_exec_count: self.engine.native_trace_exec_count,
