...
impl Vm {
    ...
    /// Validates that a callback value is a callable owned by this VM with
    /// arity one and, when its metadata is available, `fn(map) -> map` shape.
    ...
    /// Records the resume instruction pointer of the host call that admitted
    /// the stream, so a later callback completion can restore the parent
    /// frame position.
    ...
    /// Cancels the active callable stream, dropping the driver (releasing
    /// producer resources) and the owned callback/item values.
    ...
    /// Polls the active callable stream's producer, delivering items into the
    /// script callback and applying its returned action.
    ...
    /// Resumes the stream after a `run` step that halted with a callback
    /// frame on top: consumes the callback's returned action and either
    /// continues producer polling or completes the stream.
    ...
    /// Aborts the stream when a `run` step failed while a callback frame was
    /// on top, releasing interpreter state and producer resources.
