...
impl ResourceHandle {
    ...
    ///
    /// Rejects a zero raw value, a set sign bit, a zero arena identity, a zero
    /// slot identity, and a zero generation.
...
#[cfg(test)]
mod tests {
    use super::{MAX_HANDLE_ARENA_ID, ResourceHandle};

    fn pack(arena: u64, slot_identity: u64, generation: u64) -> u64 {
        (arena << 43) | (slot_identity << 25) | generation
    }

    #[test]
    fn minimum_handle_round_trips_and_is_positive() {
        let h = ResourceHandle::encode(1, 0, 1).expect("valid");
        assert_eq!(h.raw(), pack(1, 1, 1));
        assert!((h.raw() as i64) > 0);
        assert_eq!(ResourceHandle::from_raw(h.raw()).expect("decodes"), h);
        assert_eq!(h.generation(), 1);
        assert_eq!(h.slot_index().expect("slot"), 0);
    }

    #[test]
    fn decode_rejects_invalid_encodings() {
        assert!(ResourceHandle::from_raw(0).is_err());
        assert!(ResourceHandle::from_raw(pack(0, 1, 1)).is_err());
        assert!(ResourceHandle::from_raw(pack(1, 0, 1)).is_err());
        assert!(ResourceHandle::from_raw(pack(1, 1, 0)).is_err());
        assert!(
            (ResourceHandle::from_raw(pack(MAX_HANDLE_ARENA_ID, 1, 1))
                .is_ok()
                .then_some(())
                .is_some())
        );
    ...
    fn encode_rejects_out_of_range_fields() {
        assert!(ResourceHandle::encode(0, 0, 1).is_none());
        assert!(ResourceHandle::encode(MAX_HANDLE_ARENA_ID + 1, 0, 1).is_none());
        assert!(ResourceHandle::encode(1, (1 << 18) as usize, 1).is_none());
        assert!(ResourceHandle::encode(1, 0, 0).is_none());
    }
}
