...
mod tests {
    ...
    fn reasons_cover_lifecycle_vocabulary_with_raw_and_string_round_trip() {
        for (reason, raw, text) in [
            ...
            (ResourceCloseReason::VmDrop, 6, "vm_drop"),
        ] {
...
mod architecture_tests {
    ...
    fn resource_production_sources_reject_core_and_domain_imports() {
        ...
        for path in &sources {
            ...
            // Explicit external domain coupling is forbidden; this module
            // family must stay host- and domain-agnostic. Built via join so
            // the guarded token cannot accidentally appear in this very test.
