...
pub struct ResourceTable {
    ...
    /// Arena-owned typed scope state, keyed by [`TypeId`] and erased as
    /// `Box<dyn Any + Send>`.
    ///
    /// This map lives *directly on* the resource table so the owning scope can
    /// carry per-type state while its (possibly many) ordinary resource slots
    /// stay free for actual resources. It is deliberately separate from the
    /// handle/slot arena: an ordinary resource whose payload happens to be `T`
    /// can never collide with a scope-state entry also keyed by
    /// [`TypeId::of::<T>`]. Entries are dropped exactly once, when the table's
    /// terminal close reaches quiescence (see
    /// [`poll_close_all_report`](Self::poll_close_all_report)).
    scope_states: HashMap<TypeId, Box<dyn Any + Send>>,
}
...
impl ResourceTable {
    ...
    pub fn poll_close_all_report(
        ...
        if self.is_empty() {
            ...
            // Terminal resource close: drop the typed scope-state arena exactly
            // once, only after every ordinary resource has reached quiescence.
            // The map's `Drop` also runs naturally if the table itself is
            // dropped without ever reaching this path.
            self.clear_scope_states();
    ...
    ///
    /// The entry lives in the arena-owned map separate from the ordinary
    /// resource slots, so an ordinary resource whose payload is also `T` can
    /// never collide with it. The reference is valid for as long as `&mut self`
    /// because the erased [`Box`] lives directly in the table.
    pub fn scope_state_or_insert_with<T: Send + 'static, F: FnOnce() -> T>(
        ...
        self.scope_states
            ...
            .downcast_mut::<T>()
            .expect("state keyed by TypeId::of::<T> must downcast to T")
    }
    ...
    pub fn scope_state<T: Send + 'static>(&self) -> Option<&T> {
        self.scope_states
            .get(&TypeId::of::<T>())
            .and_then(|erased| erased.downcast_ref::<T>())
    }
    ...
    pub fn scope_state_mut<T: Send + 'static>(&mut self) -> Option<&mut T> {
        self.scope_states
            .get_mut(&TypeId::of::<T>())
            .and_then(|erased| erased.downcast_mut::<T>())
    }
    ...
    /// Drops every typed scope-state entry.
    ///
    /// Private: only the table's own terminal close path calls it, exactly
    /// once, so the per-type payloads are dropped deterministically rather than
    /// on arbitrary scope teardown.
    fn clear_scope_states(&mut self) {
