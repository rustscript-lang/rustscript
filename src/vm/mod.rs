    ...
    pub(super) fn execute_interpreter_instruction(
        ...
        match opcode {
            ...
            x if x == OpCode::ArraySetLocal as u8 => {
                let opcode_ip = self.ip - 1;
                let local_index = if let Some(index) = self.decoded_local_index_at(opcode_ip) {
                    self.ip += 1;
                    index
                } else {
                    self.read_u8()?
                ...
                let array_index = self.stack[operand_start].as_int()?;
                let items = match self.locals.get(local_index as usize) {
                    Some(Value::Array(items)) => items,
                    Some(_) => return Err(VmError::TypeMismatch("array")),
                    None => return Err(VmError::InvalidLocal(local_index)),
                };
                if array_index < 0 {
                ...
                let checked_index = usize::try_from(array_index)
                    .map_err(|_| VmError::HostError("array index overflow".to_string()))?;
                if checked_index > items.len() {
                    return Err(VmError::HostError(format!(
                        "array index {checked_index} out of bounds"
                    )));
                }

                let value = self.stack.pop().expect("array set value was validated");
                let _array_index = self.stack.pop().expect("array set index was validated");
                let replaced = match &mut self.locals[local_index as usize] {
                    Value::Array(items) => crate::builtins::runtime::core::array_set_in_place(
                        items,
                        array_index,
                        value,
                    )?,
                    _ => unreachable!("array local was validated before mutation"),
                };
                if let Some(replaced) = replaced {
                    self.drop_value_with_contract(replaced);
                }
            }
            x if x == OpCode::ArrayPushLocal as u8 => {
                let opcode_ip = self.ip - 1;
                let local_index = if let Some(index) = self.decoded_local_index_at(opcode_ip) {
                    self.ip += 1;
                    index
                ...
                let value = self.stack.pop().expect("array push value was validated");
                match &mut self.locals[local_index as usize] {
                    Value::Array(items) => {
                        crate::builtins::runtime::core::array_push_in_place(items, value)
                    }
                    _ => unreachable!("array local was validated before mutation"),
                }
            }
            x if x == OpCode::MapSetLocal as u8 => {
                let opcode_ip = self.ip - 1;
                let local_index = if let Some(index) = self.decoded_local_index_at(opcode_ip) {
                    self.ip += 1;
                    index
                ...
                match self.locals.get(local_index as usize) {
                    Some(Value::Map(_)) => {}
                    Some(_) => return Err(VmError::TypeMismatch("map")),
                    None => return Err(VmError::InvalidLocal(local_index)),
                }

                let value = self.stack.pop().expect("map set value was validated");
                let key = self.stack.pop().expect("map set key was validated");
                let replaced = match &mut self.locals[local_index as usize] {
                    Value::Map(entries) => {
                        crate::builtins::runtime::core::map_set_in_place(entries, key, value)
                    }
                    _ => unreachable!("map local was validated before mutation"),
                };
                if let Some(replaced) = replaced {
                    self.drop_value_with_contract(replaced);
                }
