...
/// `http-client` on non-wasm targets, SQLite under `sqlite`, and JIT config
/// under `runtime`. Required/present/stage is one opaque operation; the VM
/// core never sees a surface mask or count.
#[derive(Debug)]
