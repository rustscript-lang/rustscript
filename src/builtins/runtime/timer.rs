...
impl OwnedTimerCallback {
    ...
    pub fn set_async_bridge(&mut self, bridge: Box<dyn crate::HostAsyncBridge>) -> VmResult<()> {
        self.vm.try_set_async_bridge(bridge)
    }
    ...
    pub fn cancel(&mut self) -> VmResult<()> {
        ...
        self.vm.try_cancel_waiting_host_op()?;
        self.vm.try_shutdown()?;
...
impl HostOwnedFunction for RegisterTimer {
    fn call(&mut self, call: &mut OwnedHostCall<'_>) -> VmResult<CallOutcome> {
        ...
        let registration = TimerRegistration {
            delay,
            interval: self.repeating.then_some(delay),
...
/// Returns one installed backend count.
...
pub fn register_timer_builtin_module_from_catalog(
    ...
    let schemas = names
        .iter()
        .map(|name| {
            crate::vm::host_extension::validate_catalog_import_schemas(
                catalog,
                &timer_host_catalog(),
                name,
            )
...
mod tests {
    ...
    use super::*;
    use crate::bytecode::{
        CallableKind, CallablePrototype, CallableTarget, HostImport, Program, RootCallableBinding,
        ValueType,
    };
    use crate::{BytecodeBuilder, CallReturn, HostAsyncBridge, HostFunction, HostOpId};
    ...
    fn make_panic_once_callback() -> OwnedTimerCallback {
        ...
        program.imports = vec![HostImport {
            name: "test::panic_once".to_string(),
            arity: 0,
        ...
        program.script_functions = vec![crate::bytecode::ScriptFunction {
            entry_ip: 1,
    ...
    fn make_resume_error_once_callback() -> OwnedTimerCallback {
        ...
        program.imports = vec![
            ...
            HostImport {
                name: "test::denominator".to_string(),
                arity: 0,
    ...
    /// A callback round that reports `VmStatus::Yielded` (fuel exhaustion is
    /// the deterministic trigger) is a *live* round: it is exposed as
    /// [`TimerCallbackState::Yielded`], an overlapping round is rejected
    /// through [`OwnedTimerCallback::is_live`], and `poll` resumes it to a
    /// terminal state.
    #[test]
    fn yielded_callback_round_blocks_overlap_then_resumes_to_complete() {
        let mut callback = make_immediate_callback();
        ...
        callback.vm.add_fuel(8).expect("recharge fuel");
        let waker = std::task::Waker::from(Arc::new(NoopWake));
        let mut cx = Context::from_waker(&waker);
        assert!(matches!(
            callback.poll(&mut cx),
