        ...
        loop {
            ...
            match opcode {
                ...
                OpCode::ArraySetLocal => {
                    ...
                    let array_index: i64 = match self.stack[operand_start] {
                        Value::Int(v) => v,
                        _ => return Err(VmError::TypeMismatch("int")),
                    };
                    let Value::Array(ref items) = self
                        .locals
                        .get(index as usize)
                        .cloned()
                        .ok_or(VmError::InvalidLocal(index))?
                    ...
                    let checked_index = usize::try_from(array_index)
                        .map_err(|_| VmError::HostError("array index overflow".into()))?;
                    if checked_index > items.len() {
                        return Err(VmError::HostError("array index out of bounds"));
                    }
                    let value = self.stack.pop().ok_or(VmError::StackUnderflow)?;
                    let _ = self.stack.pop().ok_or(VmError::StackUnderflow)?;
                    let mut items = Rc::make_mut(match &mut self.locals[index as usize] {
                        Value::Array(items) => items,
                        _ => unreachable!("validated array local"),
                    });
                    if checked_index == items.len() {
                        items.push(value);
                    } else {
                        items[checked_index] = value;
                    }
                }
                OpCode::ArrayPushLocal => {
                    ...
                    match self.locals.get(index as usize) {
                        ...
                        None => return Err(VmError::InvalidLocal(index)),
                    }
                    let value = self.stack.pop().ok_or(VmError::StackUnderflow)?;
                    let items = Rc::make_mut(match &mut self.locals[index as usize] {
                        Value::Array(items) => items,
                        _ => unreachable!("validated array local"),
                    });
                    items.push(value);
                ...
                OpCode::MapSetLocal => {
                    ...
                    let entries = Rc::make_mut(match &mut self.locals[index as usize] {
                        Value::Map(entries) => entries,
                        _ => unreachable!("validated map local"),
                    });
                    if matches!(value, Value::Null) {
                        if let Some(pos) = entries.iter().position(|(k, _)| k == &key) {
                            entries.remove(pos);
                        }
                    } else if let Some((_, existing)) = entries.iter_mut().find(|(k, _)| k == &key)
                    {
                        *existing = value;
                    } else {
