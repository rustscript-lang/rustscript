...
Full audit of all `pd-*` crates. No `#[deprecated]` attributes, LEGACY/COMPAT comments, or compat feature flags exist in `pd-vm`, [pd-edge](Dockerfile.pd-edge), or `pd-edge-abi`. All found items are in [pd-controller](Dockerfile.pd-controller). One test in `pd-vm` ([assemble_rejects_legacy_opcode_literals](pd-vm/tests/vm/vm_runtime_tests.rs#437-445)) tests that old opcode names stay rejected—that is a regression guard and should **not** be removed.
