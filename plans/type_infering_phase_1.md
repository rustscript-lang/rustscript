...
- Add `TypeMap` to [src/bytecode.rs](src/bytecode.rs).
- Record inferred operand types at emitted bytecode offsets in [src/compiler/mod.rs](src/compiler/mod.rs).
...
- Persist `Program.type_map` through VMBC in [src/vmbc.rs](src/vmbc.rs).
- Persist the richer trace/AOT shape through [src/vm/jit/aot.rs](src/vm/jit/aot.rs).
- Extend host import metadata to carry explicit return types in [src/bytecode.rs](src/bytecode.rs).
...
- [tests/wire_tests.rs](tests/wire_tests.rs): round-trip `Program.type_map`
- [tests/compiler/type_inference_tests.rs](tests/compiler/type_inference_tests.rs): compiler attaches known operand types to emitted bytecode
- [tests/compiler/compiler_common_tests.rs](tests/compiler/compiler_common_tests.rs): compile/runtime sanity remains intact with the new metadata attached

## Phase 2: Use Metadata in the Interpreter and Trace JIT

### Scope

- Teach the interpreter in [src/vm/mod.rs](src/vm/mod.rs) to consume operand metadata for monomorphic fast paths.
- Avoid hot-path `HashMap` lookups by materializing a dense packed hint table once per VM.
- Teach the trace recorder in [src/vm/jit/trace.rs](src/vm/jit/trace.rs) to emit typed trace steps instead of falling back to generic arithmetic where types are known.
- Execute the typed steps in [src/vm/jit/runtime.rs](src/vm/jit/runtime.rs).
- Inline typed native lowering in [src/vm/jit/native/codegen.rs](src/vm/jit/native/codegen.rs), including float arithmetic and float comparisons/equality.
...
- [tests/jit_tests.rs](tests/jit_tests.rs): typed int/float/string trace-step coverage
- [tests/jit_tests.rs](tests/jit_tests.rs): typed float math lowers natively without helper-bridge hits
- [tests/jit_tests.rs](tests/jit_tests.rs): typed float compare/equality lowers natively without helper-bridge hits
- [tests/jit/perf_tests.rs](tests/jit/perf_tests.rs): `perf_jit_native_reduces_tight_loop_latency`
...
- [tests/compiler/compiler_common_tests.rs](tests/compiler/compiler_common_tests.rs):
  - data-driven `if/else` mismatch rejection cases
  - shadowed outer-local mismatch cases
  - loop-carried shadowed mismatch cases
- [tests/compiler/diagnostics_tests.rs](tests/compiler/diagnostics_tests.rs): rendered compile diagnostics for `if/else` mismatches
...
- [src/compiler/diagnostics.rs](src/compiler/diagnostics.rs)
