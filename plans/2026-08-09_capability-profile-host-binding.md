...
- Can be implemented alongside VM decomposition; the VM-owned part is callable authorization only.
- HTTP/IO/SQLite configuration remains state owned by each host implementation and is not embedded in the profile.
...
- A clean boundary between callable authorization and host-owned path/network/database/process limits.
...
HostFunctionState (type-erased by the VM, typed by each host module)
  HTTP configuration and in-flight permits
  IO path/process/size policy
  SQLite root and resource limits
...
4. Keep HTTP/SQLite/IO policy types out of the VM capability layer.
5. Compute a deterministic fingerprint from callable authorization only.
...
- Parameter policies remain host-owned native upper bounds that script data cannot widen.
- `HostRuntime`, `CapabilityProfile`, and inherent `Vm` APIs do not name HTTP/IO/SQLite policy types.
