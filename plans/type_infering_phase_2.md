...
Four improvements to the type inference pass in [opt.rs](src/compiler/opt.rs), ordered by impact.
...
#### [MODIFY] [opt.rs](src/compiler/opt.rs)

In [legalize_stmts](src/compiler/opt.rs#526-587), [collect_stmt_types](src/compiler/opt.rs#1093-1174), and [validate_stmts](src/compiler/opt.rs#588-664) — the three places that handle `For`/`While`:
...
The key insight: [merge_from_branches](src/compiler/opt.rs#115-139) already does the right thing — if a slot has the same type in both states, it keeps it; otherwise it becomes `Unknown`. After two iterations, a stable loop counter stays `Int`.

`While` gets the same treatment but without [init](src/compiler/mod.rs#2278-2292)/`post`.
...
- [for](src/vm/mod.rs#843-860) loop with stable `Int` counter → type preserved after loop
...
**Problem**: Several builtins return `Unknown` in [infer_call_like_expr_type](src/compiler/opt.rs#246-282) even though their return types are statically known.
...
> [!TIP]
> The namespace builtins are generated from [build.rs](build.rs). You could add a [return_type](tests/compiler/type_inference_tests.rs#174-234) field to `NamespaceMemberDecl` so the build script generates return type metadata automatically. But for now, manually expanding the match arms is simpler and lower risk.
...
**Problem**: External host functions (non-builtin `Call`) return `Unknown` because [HostImport](src/bytecode.rs#114-118) only has [name](src/compiler/opt.rs#22-34) and [arity](build.rs#413-422).

#### [MODIFY] [bytecode.rs](src/bytecode.rs)
...
> [!NOTE]
> This requires the [TypeContext](src/compiler/opt.rs#154-158) to have access to the host import table during type inference. Thread `imports: &[HostImport]` into [TypeContext](src/compiler/opt.rs#154-158), or pass the remapped host import return types as a `HashMap<u16, ValueType>` alongside the [FrontendIr](src/compiler/ir.rs#171-178).
...
**Verification**: Register a host function with `return_type: ValueType::Int`, call it in a script, and verify the result variable has inferred type `Int` in the [TypeMap](src/bytecode.rs#33-37).

---

## Part 4: `Get` Return Type from Homogeneous Containers

**Problem**: [get(array, index)](src/compiler/opt.rs#64-70) and [get(map, key)](src/compiler/opt.rs#64-70) always return `Unknown`, even when the container only holds values of one type.
...
Then in [infer_call_like_expr_type](src/compiler/opt.rs#246-282):
- `ArrayPush(arr, elem)`: if [arr](src/bytecode.rs#43-46) is `ArrayOf(T)` and `elem` is `T`, result is `ArrayOf(T)`; otherwise `Array`
...
> [!WARNING]
> This is the most complex change. The `BoundType` enum grows, [merge_from_branches](src/compiler/opt.rs#115-139) needs to handle `ArrayOf` merges (same element type → keep; different → demote to `Array`), and all downstream conversions (`BoundType → ValueType`) need updating. Consider deferring this until after Parts 1-3 show measurable wins.
...
| 4. Homogeneous container tracking | High | Medium — helps [get()](src/compiler/opt.rs#64-70) chains | Medium — enum change ripples |
