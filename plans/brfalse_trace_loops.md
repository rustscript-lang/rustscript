...
- Primary touch points: [trace.rs](src/vm/jit/trace.rs), [runtime.rs](src/vm/jit/runtime.rs), [codegen.rs](src/vm/jit/native/codegen.rs), [bridge.rs](src/vm/jit/native/bridge.rs), [aot.rs](src/vm/jit/aot.rs).
...
## Implementation Plan
1. Refactor backward-branch recording in [trace.rs](src/vm/jit/trace.rs) into one shared helper used by both `compile_trace` and `compile_aot_block`, so the two paths cannot drift again.
...
5. Update [runtime.rs](src/vm/jit/runtime.rs) so `execute_jit_trace` uses a manual `while step_index < trace.steps.len()` loop instead of `for`, builds a local `ip -> step_index` map once per trace execution, and handles `LoopIfFalse` by:
   - popping a bool,
...
7. Extend native codegen in [cranelift.rs](src/vm/jit/native/cranelift.rs) and [codegen.rs](src/vm/jit/native/codegen.rs) with a two-pass layout:
...
9. Extend [bridge.rs](src/vm/jit/native/bridge.rs) with `OP_LOOP_IF_FALSE` and bridge-name reporting. The helper semantics are: `false` => pop bool and return `STATUS_CONTINUE`; `true` => `jump_to(exit_ip)` and return `STATUS_TRACE_EXIT`; invalid stack/type => existing error path.
10. Update [aot.rs](src/vm/jit/aot.rs) encode/decode/validation for the new step tag, and validate that every `LoopIfFalse.target_ip` resolves to an earlier `step_ip` within the same trace.
...
- Replace the current NYI tests in [jit_nyi_edge_tests.rs](tests/jit/jit_nyi_edge_tests.rs) with positive tests that hand-build bytecode containing a backward `brfalse` to the trace root and assert:
...
- Keep existing loop/join-path tests green, especially the `GuardTrue` join-path case and nested-loop trace tests in [jit_tests.rs](tests/jit/jit_tests.rs).
