...
#[cfg(test)]
mod tests {
    use super::expand_scoped_pd_host_function;
    use syn::{ItemFn, Meta, Token, parse_quote, punctuated::Punctuated};

    #[test]
    fn preserves_async_scoped_edge_host_expansion() {
        let attr: Punctuated<Meta, Token![,]> = parse_quote!(name = "test::suspend", scope = http);
        let item: ItemFn = parse_quote! {
            /// Returns a value after a scoped host operation completes.
            #[pd_host_function(name = "test::suspend", scope = http)]
            async fn suspend(
                _vm: &mut Vm,
        ...
        let expanded = expand_scoped_pd_host_function(attr, item)
            .expect("async scoped edge host functions should be accepted");
        let rendered = expanded.to_string();
        assert!(rendered.contains("schedule_current_future_call"));
        assert!(rendered.contains("PD_EDGE_HOST_FUNCTIONS"));
    }
}
