...
impl Vm {
    ...
    pub(super) fn string_len_op(&mut self) -> VmResult<()> {
        let text = match self.pop_value()? {
            Value::String(value) => value,
            _ => return Err(VmError::TypeMismatch("string")),
        };
        self.stack.push(Value::Int(text.chars().count() as i64));
        Ok(())
    }

    pub(super) fn bytes_len_op(&mut self) -> VmResult<()> {
    ...
    pub(super) fn string_slice_op(&mut self) -> VmResult<()> {
        ...
        let sliced = match checked_slice_bounds(start, length)? {
            Some((start, length)) => text.chars().skip(start).take(length).collect::<String>(),
            None => String::new(),
        };
    ...
    pub(super) fn bytes_slice_op(&mut self) -> VmResult<()> {
        let length = self.pop_int()?;
        let start = self.pop_int()?;
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
        ...
        self.stack.push(Value::bytes(sliced));
        Ok(())
    }
    ...
    pub(super) fn string_get_op(&mut self) -> VmResult<()> {
    ...
    pub(super) fn bytes_get_op(&mut self) -> VmResult<()> {
        let index = self.pop_int()?;
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
    ...
    pub(super) fn bytes_has_op(&mut self) -> VmResult<()> {
        let index = self.pop_int()?;
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
    ...
    pub(super) fn bytes_from_utf8_op(&mut self) -> VmResult<()> {
        let text = match self.pop_value()? {
            Value::String(value) => value,
            _ => return Err(VmError::TypeMismatch("string")),
        };
        self.stack.push(Value::bytes(text.as_bytes().to_vec()));
        Ok(())
    }

    pub(super) fn bytes_to_utf8_op(&mut self) -> VmResult<()> {
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
        };
        let text = std::str::from_utf8(bytes.as_slice())
            .map(str::to_string)
            .map_err(|err| {
                VmError::HostError(format!("bytes::to_utf8 requires valid utf-8: {err}"))
            })?;
        self.stack.push(Value::string(text));
        Ok(())
    }

    pub(super) fn bytes_to_utf8_lossy_op(&mut self) -> VmResult<()> {
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
        };
    ...
    pub(super) fn bytes_from_hex_op(&mut self) -> VmResult<()> {
        let text = match self.pop_value()? {
            Value::String(value) => value,
            _ => return Err(VmError::TypeMismatch("string")),
        };
    ...
    pub(super) fn bytes_to_hex_op(&mut self) -> VmResult<()> {
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
        };
    ...
    pub(super) fn bytes_from_base64_op(&mut self) -> VmResult<()> {
        let text = match self.pop_value()? {
            Value::String(value) => value,
            _ => return Err(VmError::TypeMismatch("string")),
        };
        let decoded = STANDARD.decode(text.as_str()).map_err(|err| {
            VmError::HostError(format!("bytes::from_base64 requires valid base64: {err}"))
        })?;
        self.stack.push(Value::bytes(decoded));
        Ok(())
    }

    pub(super) fn bytes_to_base64_op(&mut self) -> VmResult<()> {
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
        };
        self.stack
            .push(Value::string(STANDARD.encode(bytes.as_slice())));
        Ok(())
    }

    pub(super) fn bytes_from_array_u8_op(&mut self) -> VmResult<()> {
        let values = match self.pop_value()? {
            Value::Array(value) => crate::bytecode::unwrap_or_clone_shared(value),
    ...
    pub(super) fn bytes_to_array_u8_op(&mut self) -> VmResult<()> {
        let bytes = match self.pop_value()? {
            Value::Bytes(value) => value,
            _ => return Err(VmError::TypeMismatch("bytes")),
        };
