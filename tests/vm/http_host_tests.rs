    ...
    for status in [301, 302] {
        for method in ["POST", "PUT", "PATCH", "DELETE", "OPTIONS"] {
            ...
            assert!(
                first
                    .to_ascii_lowercase()
                    .starts_with(&format!("{} /start http/1.1", method.to_ascii_lowercase())),
                "status {status}, method {method}: {first}"
            );
            let expected_method = if method == "POST" { "GET" } else { method };
            assert!(
                second.to_ascii_lowercase().starts_with(&format!(
                    "{} /final http/1.1",
                    expected_method.to_ascii_lowercase()
                )),
                "status {status}, method {method}: {second}"
            );
            if method == "POST" {
