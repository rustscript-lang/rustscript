...
use vm::{
    CallableKind, CallablePrototype, CallableTarget, FunctionRegion, MAX_FRAME_LOCAL_COUNT,
    Program, ScriptFunction, Value, VmError, VmStatus,
};
...
fn direct_program_oversized_root_frame_does_not_allocate_or_run() {
    let program = Program::new(Vec::new(), vec![0x01]).with_local_count(1_000_000);
    assert!(matches!(
...
#[test]
fn direct_program_oversized_callee_frame_does_not_allocate_or_run() {
    let mut program = call_script_program(
        0,
        0,
        0,
        CallableTarget::ScriptFunction(0),
    ...
    program.callable_prototypes[0].frame_local_count = 1_000_000;
    let mut vm = Vm::new(program);
