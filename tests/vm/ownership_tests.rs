...
fn jit_inlined_callee_root_callable_escapes_through_host_stash() {
    ...
    {
        let outcome = vm
            .submit_callable_stream(first.clone(), ImmediateCompleteDriver)
            .expect("the stream callback gate must accept the escaped callable");
        assert!(matches!(outcome, vm::CallOutcome::Pending(_)));
    }
    ...
    assert!(
        matches!(
            vm.submit_callable_stream(first.clone(), ImmediateCompleteDriver),
            Err(VmError::InvalidCallable)
        ),
        "stale handle must be rejected by the stream callback gate"
    );
    ...
    assert!(
        matches!(
            vm.submit_callable_stream(second.clone(), ImmediateCompleteDriver),
            Ok(vm::CallOutcome::Pending(_))
        ),
        "the second run's escaped callable must pass the stream callback gate"
    );
    assert!(
        matches!(
            vm.start_callable(first.clone(), &[Value::map(Vec::new())]),
...
/// A stream driver that completes immediately; only the ownership gate of
/// `submit_callable_stream` is under test here, so the driver never polls.
struct ImmediateCompleteDriver;

impl vm::HostStreamDriver for ImmediateCompleteDriver {
    fn poll_next(
        &mut self,
        _cx: &mut std::task::Context<'_>,
    ) -> std::task::Poll<Result<vm::HostStreamPoll, vm::VmError>> {
        std::task::Poll::Ready(Ok(vm::HostStreamPoll::Complete(Value::map(Vec::new()))))
    }

    fn apply_action(&mut self, _action: Value) -> Result<vm::HostStreamAction, vm::VmError> {
        Ok(vm::HostStreamAction::Continue)
    }
}
