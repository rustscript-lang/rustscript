...
#[test]
fn collection_local_array_set_replace_append_and_push_use_unique_arc() {
    let local = Value::array(vec![Value::Int(10), Value::Int(20)]);
    let Value::Array(local_array) = &local else {
        unreachable!();
    };
    let original_ptr = Arc::as_ptr(local_array);
    ...
    bc.ldc(0);
    bc.ldc(1);
    bc.array_set_local(0);
    bc.ldc(2);
    bc.ldc(3);
    bc.array_set_local(0);
    bc.ldc(4);
    bc.array_push_local(0);
    bc.ldloc(0);
    bc.ret();

    let mut vm = Vm::new(Program::new(constants, bc.finish()));
    vm.set_local(0, local).expect("local should exist");
    ...
    let Value::Array(result) = &vm.locals()[0] else {
        panic!("expected array local");
    };
    assert_eq!(Arc::as_ptr(result), original_ptr);
    assert_eq!(vm.stack(), vm.locals());
}

#[test]
fn collection_local_map_set_inserts_overwrites_and_null_removes() {
    let constants = vec![
        Value::string("a"),
        Value::Int(2),
        Value::string("b"),
    ...
    let mut vm = Vm::new(Program::new(constants, bc.finish()));
    vm.set_local(0, Value::map(vec![(Value::string("a"), Value::Int(1))]))
        .expect("local should exist");
    assert_eq!(vm.run().expect("vm should run"), VmStatus::Halted);

    let Value::Map(result) = &vm.locals()[0] else {
        panic!("expected map local");
    };
    assert_eq!(result.get(&Value::string("a")), None);
    assert_eq!(result.get(&Value::string("b")), Some(&Value::Int(3)));
    assert!(vm.stack().is_empty());
}

#[test]
fn collection_local_shared_arc_detaches_before_mutation() {
    ...
    let mut vm = Vm::new(Program::new(vec![Value::Int(2)], bc.finish()));
    vm.set_local(0, local).expect("local should exist");
    assert_eq!(vm.run().expect("vm should run"), VmStatus::Halted);
    assert_eq!(
        vm.locals(),
    ...
    assert_eq!(alias, Value::array(vec![Value::Int(1)]));
    let (Value::Array(result), Value::Array(alias)) = (&vm.locals()[0], &alias) else {
...
#[test]
fn collection_local_array_errors_leave_target_unchanged() {
    let cases = [
        (
            Value::Int(-1),
            vm::VmError::HostError("array index must be non-negative".into()),
        ),
        (
            Value::Int(3),
            vm::VmError::HostError("array index 3 out of bounds".into()),
        ),
        (Value::string("bad"), vm::VmError::TypeMismatch("int")),
    ];
    ...
    for (index, expected) in cases {
        ...
        bc.array_set_local(0);
        bc.ret();
        let mut vm = Vm::new(Program::new(vec![index, Value::Int(9)], bc.finish()));
        let original = Value::array(vec![Value::Int(1), Value::Int(2)]);
        vm.set_local(0, original.clone())
            .expect("local should exist");
        let actual = vm.run().expect_err("array set should fail");
        assert_eq!(actual.to_string(), expected.to_string());
        assert_eq!(vm.locals(), &[original]);
    }
    ...
    vm.set_local(0, Value::Int(7)).expect("local should exist");
    let err = vm.run().expect_err("wrong local type should fail");
    assert!(matches!(err, vm::VmError::TypeMismatch("array")));
    assert_eq!(vm.locals(), &[Value::Int(7)]);
}
