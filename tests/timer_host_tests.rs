...
fn at_compile_contract_uses_callable_take_owned() {
    let compiled = compile_standard("use timer; timer::at(10, |premature| null);")
        .expect("timer::at must compile");
    let index = compiled
        .program
        .imports
        .iter()
        .position(|import| import.name == "timer::at")
        .expect("timer::at exact import");
    let schema = compiled
        .program
        .host_import_schemas()
        .get(index)
        .expect("aligned import schema slot")
        .as_ref()
        .expect("catalog-resolved schema");
    assert_eq!(schema.params[1].passing, HostParamPassing::TakeOwned);
}

/// The typed callback surface declares its result as `unknown` because the
/// module discards it: an `int`/`string`/`map`/`bool`/`null` callback body all
/// compile against the standard catalog and run to completion, and the
/// discarded value never reaches the guest.
#[test]
fn callbacks_with_any_declared_result_compile_and_discard_their_result() {
    for (name, body) in [
        ("int", "1"),
        ("string", "\"text\""),
        ("map", "{a: 1}"),
        ...
        let mut vm = run_with_backend(
            &format!("use timer; timer::at(5, |premature| {body});"),
            Arc::clone(&backend),
            TimerConfig::default(),
        )
        .unwrap_or_else(|error| {
            panic!("the {name}-returning callback must compile and register: {error}")
        });
        assert_eq!(
            vm.take_callable_result(),
            None,
            "the registration call itself leaves no callback result on the source VM"
        );
        ...
        for round in 1..=2 {
            assert_eq!(
                ...
                "the {name}-returning callback must run to completion in round {round}"
            );
        }
    }
}
...
fn at_registers_and_returns_then_callback_runs() {
    let backend = Arc::new(ManualBackend::default());
    let mut vm = run_with_backend(
        "use timer; timer::at(25, |premature| null);",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
...
fn at_callback_observes_a_non_premature_round() {
    let backend = Arc::new(ManualBackend::default());
    run_with_backend(
        "use timer; timer::at(10, |premature| if true => { assert(!premature); null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn every_marks_interval_and_preserves_callable_between_rounds() {
    let backend = Arc::new(ManualBackend::default());
    run_with_backend(
        "use timer; timer::every(7, |premature| null);",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn every_preserves_captured_callable_graph_across_rounds() {
    ...
    let source_vm = run_with_backend(
        "use timer; let expected = false; timer::every(1, |premature| if true => { assert(expected == premature); null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
...
fn every_rounds_share_one_mutable_capture_cell() {
    ...
    let (_vm, recorded) = run_with_test_recorder(
        "use test;\nuse timer;\nlet mut rounds = 0;\ntimer::every(5, |premature| if true => { rounds = rounds + 1; test::record(rounds); null } else => { test::record(rounds); null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
...
fn counts_are_synchronous_and_reflect_backend_state() {
    ...
    *backend.running.lock().expect("running") = 3;
    let vm = run_with_backend(
        "use timer; timer::at(1, |premature| null); timer::pending_count() + timer::running_count();",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
...
fn invalid_durations_and_pending_limit_are_rejected() {
    for source in [
        "use timer; let callback = |premature| null; timer::at(-1, callback);",
        "use timer; let callback = |premature| null; timer::every(0, callback);",
    ] {
        let backend = Arc::new(ManualBackend::default());
    ...
    let mut vm = build_vm_with_backend(
        "use timer; let callback = |premature| null; timer::at(2, callback);",
        &backend,
        TimerConfig {
            max_pending: 1,
            max_running: 1,
...
fn scheduler_leaves_due_callback_pending_until_running_slot_is_free() {
    ...
    run_with_test_wait_backend(
        "use test;\nuse timer;\nfn wait() -> bool { test::wait() }\ntimer::every(1, |premature| if true => { wait(); null } else => { null });\ntimer::every(1, |premature| if true => { wait(); null } else => { null });\n",
        Arc::clone(&backend),
        config,
    )
...
fn scheduler_survives_callback_panic_and_repeats_after_recovery() {
    let backend = Arc::new(LimitedSchedulerBackend::new(1));
    run_with_test_wait_backend(
        "use test;\nuse timer;\nfn invoke_panic() -> bool { test::panic() }\ntimer::every(1, |premature| if true => { invoke_panic(); null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig {
            max_pending: 4,
            max_running: 1,
...
fn backend_installs_independent_bridges_for_waiting_timer_callbacks() {
    let backend = Arc::new(ManualBackend::default());
    run_with_test_wait_backend(
        "use test;\nuse timer;\nfn wait() -> bool { test::wait() }\ntimer::at(1, |premature| if true => { wait(); null } else => { null });\ntimer::at(1, |premature| if true => { wait(); null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
...
fn callback_errors_go_to_backend_sink_without_touching_source_vm() {
    let backend = Arc::new(ManualBackend::default());
    let source_vm = run_with_backend(
        "use timer; timer::at(1, |premature| if true => { let unused = 1 / 0; null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn every_callback_can_report_each_round_after_an_error() {
    let backend = Arc::new(ManualBackend::default());
    run_with_backend(
        "use timer; timer::every(1, |premature| if true => { let unused = 1 / 0; null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn source_vm_drop_leaves_pending_callbacks_until_explicit_shutdown() {
    ...
    let vm = run_with_backend(
        "use timer; timer::at(100, |premature| if true => { assert(premature); null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn every_shutdown_runs_once_premature_and_stops_rescheduling() {
    let backend = Arc::new(ShutdownBackend::default());
    run_with_backend(
        "use timer; timer::every(50, |premature| if true => { assert(premature); null } else => { null });",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn concurrent_backend_admission_allows_only_the_configured_pending_count() {
    ...
    for source in [
        "use timer; timer::at(1, |premature| null);",
        "use timer; timer::at(2, |premature| null);",
    ] {
...
fn timer_callback_with_a_dropped_backend_reports_shutdown() {
    let backend = Arc::new(ManualBackend::default());
    let vm = run_with_backend(
        "use timer;\nfn count() -> int { timer::pending_count() }\ntimer::at(1, |premature| if true => { count(); null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("registration succeeds");
...
fn completed_callback_vm_is_released_exactly_once() {
    ...
    let source_vm = run_with_probe(
        "use test;\nuse timer;\ntimer::at(1, |premature| if true => { test::probe(); null } else => { test::probe(); null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
        Arc::clone(&counters),
    )
...
fn error_rounds_reuse_one_callback_vm_released_exactly_once() {
    ...
    let backend = Arc::new(ManualBackend::default());
    let source_vm = run_with_probe(
        "use test;\nuse timer;\ntimer::every(1, |premature| if true => { test::probe(); let unused = 1 / 0; null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
        Arc::clone(&counters),
    )
...
fn cancelled_waiting_callback_releases_its_vm_exactly_once() {
    ...
    let backend = Arc::new(ManualBackend::default());
    let source_vm = run_with_probe(
        "use test;\nuse timer;\nfn wait() -> bool { test::wait() }\ntimer::at(1, |premature| if true => { test::probe(); wait(); null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
        Arc::clone(&counters),
    )
...
// ---------------------------------------------------------------------------
// Downstream thin-adapter surface
// ---------------------------------------------------------------------------
//
// An embedding that exposes the standard timer contract under its *own* exact
// host name and schema (for example a seconds-based `ngx.timer`-shaped host)
// depends on three public things:
//
// * `HostOwnedFunction` / `OwnedHostCall` / `OwnedHostContext`, so it can write
//   its own owned registration adapter,
// * `register_owned_timer`, so the callback handoff, fresh callback VM,
//   admission limits, backend registration and rollback are shared with
//   `timer::{at,every}` instead of re-implemented (and so a bare
//   `OwnedTimerCallback` can never be constructed by the embedding), and
// * `installed_timer_counts`, so its count functions report the installed
//   backend's own counters without naming backend internals.
//
// The fixtures below are exactly that adapter: a two-argument seconds-based
// registration (`vendor::at`) plus a seconds-based register whose callback
// parameter is `unknown` (so the runtime type check, not the compiler, decides),
// and dependency-free count functions (`vendor::pending` / `vendor::running`).

/// A seconds-based downstream registration adapter.
struct VendorTimerRegister {
    registry: HostFunctionRegistry,
    name: &'static str,
    repeating: bool,
}

impl HostOwnedFunction for VendorTimerRegister {
    fn call(&mut self, call: &mut OwnedHostCall<'_>) -> VmResult<CallOutcome> {
        ...
        if seconds < 0 || (self.repeating && seconds == 0) {
            ...
            return Err(VmError::HostError(format!(
                "{} expects a {requirement} second duration, got {seconds}",
                self.name
            )));
        }
...
fn vendor_pending_count(vm: &mut Vm, _args: &[Value]) -> VmResult<CallOutcome> {
    vendor_count(vm, true)
}

fn vendor_running_count(vm: &mut Vm, _args: &[Value]) -> VmResult<CallOutcome> {
    vendor_count(vm, false)
}

fn vendor_count(vm: &mut Vm, pending: bool) -> VmResult<CallOutcome> {
    let counts: TimerCounts = installed_timer_counts(vm)?;
    ...
    let count = i64::try_from(count)
        .map_err(|_| VmError::HostError("vendor count exceeds int range".to_string()))?;
    Ok(CallOutcome::Return(CallReturn::one(Value::Int(count))))
}

struct VendorTimerExtension {
    timer: TimerExtension,
    catalog: Arc<HostApiCatalog>,
}

impl vm::HostExtension for VendorTimerExtension {
    ...
    fn register(&self, registry: &mut HostFunctionRegistry) -> VmResult<()> {
        vm::register_timer_builtin_module_from_catalog(registry, self.catalog.as_ref())?;
        for schema in vm::catalog_import_schemas(self.catalog.as_ref(), "vendor::at") {
            let factory = |context: OwnedHostContext<'_>| -> Box<dyn HostOwnedFunction> {
                Box::new(VendorTimerRegister {
                    registry: context.registry().clone(),
                    name: "vendor::at",
                    repeating: false,
                })
            };
            registry.register_exact_owned("vendor::at", 2, schema, factory)?;
        }
        for schema in vm::catalog_import_schemas(self.catalog.as_ref(), "vendor::every") {
            let factory = |context: OwnedHostContext<'_>| -> Box<dyn HostOwnedFunction> {
                Box::new(VendorTimerRegister {
                    registry: context.registry().clone(),
                    name: "vendor::every",
            ...
            registry.register_exact_owned("vendor::every", 2, schema, factory)?;
        }
        for schema in vm::catalog_import_schemas(self.catalog.as_ref(), "vendor::register") {
            let factory = |context: OwnedHostContext<'_>| -> Box<dyn HostOwnedFunction> {
                Box::new(VendorTimerRegister {
                    registry: context.registry().clone(),
            ...
            registry.register_exact_owned("vendor::register", 2, schema, factory)?;
        }
        let cursor = vm::catalog_import_schemas(self.catalog.as_ref(), "vendor::pending");
        for schema in cursor {
            registry.register_exact_static("vendor::pending", 0, schema, vendor_pending_count)?;
        }
        for schema in vm::catalog_import_schemas(self.catalog.as_ref(), "vendor::running") {
            registry.register_exact_static("vendor::running", 0, schema, vendor_running_count)?;
        }
        registry.authorize_registered_builtin_import("vendor::at");
        Ok(())
    }
...
/// The standard catalog plus the downstream adapter's own exact names.
fn vendor_catalog() -> Arc<HostApiCatalog> {
    let standard = standard_host_catalog();
    let mut builder = HostApiBuilder::new();
    for resource in standard.resources() {
        builder.resource(resource.clone());
    ...
    for name in ["vendor::at", "vendor::every"] {
        builder.function(HostFunctionSchema::with_return(
            name,
            vec![
                HostParamSchema::value("seconds", HostTypeSchema::Int),
    ...
    builder.function(HostFunctionSchema::with_return(
        ...
        vec![
            HostParamSchema::value("seconds", HostTypeSchema::Int),
            // The adapter accepts any value and validates the callback itself,
            // so the runtime type check (not the compiler) decides.
            HostParamSchema::value("callback", HostTypeSchema::Unknown),
        ],
        HostTypeSchema::Bool,
    ));
    for name in ["vendor::pending", "vendor::running"] {
        builder.function(HostFunctionSchema::with_return(
            name,
            Vec::new(),
            HostTypeSchema::Int,
    ...
    Arc::new(builder.build().expect("vendor catalog must be valid"))
}

/// Compiles against the vendor catalog and installs the vendor adapter without
/// running the program.
fn build_vm_with_vendor<B: TimerBackend>(
    source: &str,
    backend: &Arc<B>,
    config: TimerConfig,
) -> VmResult<Vm> {
    ...
    .expect("vendor script must compile");
    let mut vm = Vm::try_new(compiled.program)?;
    let runtime: Arc<dyn TimerBackend> = backend.clone();
    vm.install_extension(&VendorTimerExtension {
        timer: TimerExtension::new(runtime, config),
        catalog,
    })?;
    Ok(vm)
...
fn run_with_vendor<B: TimerBackend>(
    source: &str,
    backend: Arc<B>,
    config: TimerConfig,
) -> VmResult<Vm> {
    let mut vm = build_vm_with_vendor(source, &backend, config)?;
    assert_eq!(vm.run()?, VmStatus::Halted);
    Ok(vm)
}

#[test]
fn downstream_adapter_registers_through_the_shared_backend_path() {
    let backend = Arc::new(ManualBackend::default());
    let config = TimerConfig {
        max_pending: 7,
        max_running: 3,
    };
    let vm = run_with_vendor(
        "use vendor;\nvendor::at(3, |premature| null);\n",
        Arc::clone(&backend),
        config,
    )
    .expect("the downstream adapter registers through the shared path");
    assert_eq!(backend.pending_count(), 1);
    let mut registration = backend.pop();
    assert_eq!(registration.delay, Duration::from_secs(3));
    assert_eq!(registration.interval, None);
    assert_eq!(
        registration.max_pending, 7,
        "the installed limits travel with the downstream registration"
    );
    assert_eq!(registration.max_running, 3);
    assert_eq!(
        registration.callback.start(false).expect("callback runs"),
        vm::TimerCallbackStatus::Complete,
        "the callback runs on its own isolated VM"
    );
    drop(vm);
}

#[test]
fn downstream_adapter_seconds_interval_repeats_on_the_same_owned_callback() {
    let backend = Arc::new(ManualBackend::default());
    run_with_vendor(
        "use vendor;\nlet mut rounds = 0;\nvendor::every(2, |premature| if true => { rounds = rounds + 1; null } else => { null });\n",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("a repeating downstream registration is accepted");
    let mut registration = backend.pop();
    assert_eq!(registration.delay, Duration::from_secs(2));
    assert_eq!(registration.interval, Some(Duration::from_secs(2)));
    for round in 1..=2 {
        assert_eq!(
            registration.callback.start(false).expect("round"),
            vm::TimerCallbackStatus::Complete,
            "round {round} runs"
        );
    }
...
#[test]
fn downstream_adapter_rejects_a_negative_duration_before_the_handoff() {
    let backend = Arc::new(ManualBackend::default());
    let mut vm = build_vm_with_vendor(
        "use vendor;\nlet callback = |premature| null;\nvendor::at(-1, callback);\n",
        &backend,
        TimerConfig::default(),
    )
    .expect("vm setup");
    let error = vm.run().expect_err("a negative second duration must fail");
    assert!(
        error.to_string().contains("non-negative second duration"),
        "{error}"
    );
    assert_eq!(backend.pending_count(), 0);
    assert!(
        vm.stack()
            .iter()
            .any(|value| matches!(value, Value::Callable(_))),
        "the rejected call must keep its callback value"
    );
}

#[test]
fn downstream_adapter_rejects_a_zero_repeating_interval() {
    let backend = Arc::new(ManualBackend::default());
    let mut vm = build_vm_with_vendor(
        "use vendor;\nlet callback = |premature| null;\nvendor::every(0, callback);\n",
        &backend,
        TimerConfig::default(),
    )
    .expect("vm setup");
    let error = vm.run().expect_err("a zero interval must fail");
    assert!(
        error.to_string().contains("positive second duration"),
        "{error}"
    );
    assert_eq!(backend.pending_count(), 0);
}
...
#[test]
fn downstream_adapter_backend_rejection_and_panic_restore_the_callback() {
    let backend = Arc::new(RejectingBackend::default());
    let mut vm = build_vm_with_vendor(
        "use vendor;\nvendor::at(1, |premature| null);\n",
        &backend,
        TimerConfig::default(),
    )
    .expect("vm setup before rejection");
    let error = vm.run().expect_err("the backend rejects the registration");
    assert!(error.to_string().contains("backend rejected"), "{error}");
    assert_eq!(backend.attempts.load(Ordering::SeqCst), 1);
    assert!(
        vm.stack()
            .iter()
            .any(|value| matches!(value, Value::Callable(_))),
        "backend rejection must restore the callback argument"
    );
    ...
    .expect("vm setup before panic");
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| vm.run()));
    assert!(result.is_err(), "the backend panic must unwind");
    assert_eq!(backend.attempts.load(Ordering::SeqCst), 1);
    assert!(
        vm.stack()
            .iter()
            .any(|value| matches!(value, Value::Callable(_))),
        "the panic path must restore the callback argument"
    );
}

#[test]
fn downstream_adapter_without_installed_runtime_reports_missing_backend() {
    let catalog = vendor_catalog();
    let compiled = compile_source_with_flavor_and_options(
        "use vendor;\nvendor::at(1, |premature| null);\n",
        SourceFlavor::RustScript,
    ...
    .expect("vendor script must compile");
    let mut registry = HostFunctionRegistry::empty();
    let extension = VendorTimerExtension {
        // The registration surface is installed; the runtime state is not.
        timer: TimerExtension::new(Arc::new(ManualBackend::default()), TimerConfig::default()),
        catalog,
    };
    vm::register_host_extension(&mut registry, &extension).expect("vendor registration");
    let mut vm = Vm::try_new(compiled.program).expect("vm construction");
    registry
        .bind_vm_cached(&mut vm)
        .expect("bind vendor registry");
    let error = vm
        .run()
        .expect_err("a downstream call without runtime state must fail");
    assert!(
        error.to_string().contains("timer runtime is not installed"),
        "unexpected error: {error}"
    );
...
#[test]
fn downstream_adapter_callback_argument_must_be_callable() {
    // `vendor::register` declares an `unknown` callback parameter, so the
    // runtime type check (not the compiler) decides: a non-callable value must
    // be rejected and returned to the guest.
    let backend = Arc::new(ManualBackend::default());
    let mut vm = build_vm_with_vendor(
        "use vendor;\nvendor::register(1, 5);\n",
        &backend,
    ...
    .expect("vm setup");
    let error = vm.run().expect_err("a non-callable callback must fail");
    assert!(
        matches!(&error, VmError::TypeMismatch(expected) if *expected == "callable"),
        "unexpected error: {error}"
    );
    assert_eq!(backend.pending_count(), 0);
    assert!(
        vm.stack().contains(&Value::Int(5)),
        "the rejected callback value must be restored to the guest"
    );
}

#[test]
fn downstream_count_functions_report_the_installed_backend_synchronously() {
    let backend = Arc::new(ManualBackend::default());
    *backend.running.lock().expect("running") = 4;
    let vm = run_with_vendor(
        "use vendor;\nvendor::at(1, |premature| null);\nvendor::pending() + vendor::running();\n",
        Arc::clone(&backend),
        TimerConfig::default(),
    )
    .expect("the downstream count functions resolve");
    assert_eq!(vm.stack().last(), Some(&Value::Int(5)));
}

#[test]
fn downstream_count_functions_fail_without_an_installed_runtime() {
    let catalog = vendor_catalog();
    let compiled = compile_source_with_flavor_and_options(
        "use vendor;\nvendor::pending();\n",
        SourceFlavor::RustScript,
    ...
    let error = vm
        .run()
        .expect_err("counts without an installed runtime must fail");
    assert!(
        error.to_string().contains("timer runtime is not installed"),
        "unexpected error: {error}"
    );
